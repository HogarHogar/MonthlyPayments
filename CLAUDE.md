...
- **Revised estimate**: after completing a research/reading phase (i.e. after all file reads, greps, and exploration needed to understand the scope of work), if the remaining work now looks significantly different from the original overall estimate, output `⏳⏳REVISED ESTIMATED TIME ≈ Xm⏳⏳` on its own line with a brief reason for the change (e.g. "~12 files to edit, not 6 as initially expected"). This appears immediately after the research phase ends — typically right after the `⏱️` line that closes the RESEARCHING phase and before the next phase bookend. **Only output when the revised estimate differs from the original by ≥1 minute** — skip if the original estimate still looks accurate. The revised estimate replaces the original as the baseline for the ACTUAL TIME comparison at the end
...
| `⏳⏳REVISED ESTIMATED TIME ≈ Xm⏳⏳` | Estimate changed ≥1m after research | After research phase `⏱️`, before next bookend | — | — |
...
🔄🔄NEXT PHASE🔄🔄 [01:15:26 AM EST] — applying changes
  ... applying changes ...
  ⏱️ 2m 4s
